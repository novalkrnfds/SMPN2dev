<div class="wrapper">
    <div class="inner light">
		<div class="page-intro clearfix">
			<h1 class="page-title">Struktur Organisasi</h1>
		</div>
		
		<!-- Begin Content -->
		<section class="content">
			<h4 style="text-align:center">STRUKTUR ORGANISASI</h4>
			<h2 style="text-align:center">SMPN 2 SUNGAI PENUH</h2>
			<!-- <div id="strukturorganisasi" style="width: 100%; height: 480px; border-style: dotted; border-width: 1px;" />
			End Form --> 
			<?php
			$no= 1;
			if($struktur)
				foreach($struktur as $b) :
		
			?>
			<div style="width: 100%; height: 480px; border-style: dotted; border-width: 1px;">
			<img src="<?php echo base_url()?>/admin/assets/uploads/struktur/<?php echo $b->struktur; ?>">
			</div>
			


			<?php endforeach; ?>			
        
		</section>
		<!-- End Content --> 
      
		<!-- Begin Sidebar 
		<aside class="sidebar">
			<div class="sidebox">
				<br>
				<br>
				<br>
				<p style="text-align:justify;">
					SMP Negeri 2 Sungai Penuh sejak berdiri tanggal 25 Mei 1960 bernama SGB sampai berubah menjadi SMP Negeri 2 Sungai Penuh, 
					Sejak berdirinya Tahun 1960 sampai sekarang SMP Negeri 2 Sungai Penuh  berkedudukan di Sungai Penuh ibu kota Kabupaten Kerinci 
					sekarang berubah nama Kota Sungai Penuh, tepatnya di Jl.Jend. Sudirman  Kecamatan Sungai Penuh Kota Sungai Penuh.
				</p>
				<i class="icon-location contact"></i> Jl. Jendral Sudirman, Kota Sungai Penuh <br />
				<i class="icon-phone contact"></i> (0748) 21 068 <br />
				<i class="icon-mail contact"></i> <a href="mailto:info@smpn2spn.sch.id">info@smpn2spn.sch.id</a> 
			</div>
		</aside>
		<!-- End Sidebar -->
      
		<div class="clear"></div>
    </div>
</div>
 